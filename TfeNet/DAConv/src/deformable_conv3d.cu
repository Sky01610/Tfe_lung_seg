...
__device__ scalar_t DAConv_im2col_trilinear(
...
template<typename scalar_t>
__device__ void rotate_point(scalar_t &x, scalar_t &y, scalar_t &z, 
                                const scalar_t theta_x, const scalar_t theta_y, const scalar_t theta_z,int axis) {
    if (axis == 0)
    {
      // 绕Y,Z轴旋转
      scalar_t x_temp = x;
      x = x_temp*cosf(theta_y)*cosf(theta_z);
      y = x_temp*cosf(theta_y)*sinf(theta_z);
      z = -x_temp*sinf(theta_y);
    }
    else if (axis == 1)
    {
      // 绕X,Z轴旋转
      scalar_t y_temp = y;
      x = -y_temp*cosf(theta_x)*sinf(theta_z);
      y = y_temp*cosf(theta_x)*cosf(theta_z);
      z = y_temp*sinf(theta_x);
    }
    else if (axis == 2)
    {
      // 绕X,Y轴旋转
      scalar_t z_temp = z;
      x = z_temp*cosf(theta_x)*sinf(theta_y);
      y = -z_temp*sinf(theta_x);
      z = z_temp*cosf(theta_x)*cosf(theta_y);
    }
}

template <typename scalar_t>
__global__ void DAConv_im2col_gpu_kernel(
		const int n, const scalar_t *data_im,scalar_t *angle_data,
        ...
        const int height_col, const int width_col, const int length_col,
         scalar_t *data_col,const int axis)
{
...
//  columns = {channels * kernel_h * kernel_w * kernel_l,step * height_out * width_out * length_out}
    ...
    scalar_t *angle_data_ptr = angle_data
    		+(b_col * deformable_group + deformable_group_index)
    		* 4 * height_col * width_col * length_col;// 1 18 100 100

    // 获取当前batch和channel组合的角度参数
    int data_offset_1_ptr = 0 * height_col * width_col * length_col + (h_col* width_col * length_col+ w_col* length_col + l_col);
    int data_offset_2_ptr = 1 * height_col * width_col * length_col + (h_col* width_col* length_col + w_col* length_col + l_col);
    int data_offset_3_ptr = 2 * height_col * width_col * length_col + (h_col* width_col* length_col + w_col* length_col + l_col);
    int data_offset_4_ptr = 3 * height_col * width_col * length_col + (h_col* width_col* length_col + w_col* length_col + l_col);

    const scalar_t angle_1 = angle_data_ptr[data_offset_1_ptr]; // 第一个角度参数
    const scalar_t angle_2 = angle_data_ptr[data_offset_2_ptr]; // 第二个角度参数
    const scalar_t angle_3 = angle_data_ptr[data_offset_3_ptr]; // 第一个角度参数
    const scalar_t angle_4 = angle_data_ptr[data_offset_4_ptr]; // 第二个角度参数

    int kernel_num = kernel_h*kernel_w*kernel_l;
    int center = (kernel_num - 1) / 2;
    int count = 0;
    scalar_t zero = 0;
    ...
    for (int k=0;k<kernel_l;++k)
    {
      ...
      if ( axis == 0 ) // x轴
      {
        scalar_t offset1 = (count-center)*dilation_h;
        scalar_t offset2 = 0;
        scalar_t offset3 = 0;
        if (count == center)
          offset1 = 0;
        else if (count < center)
          rotate_point(offset1,offset2,offset3,zero,angle_1,angle_2,axis);
        else if (count > center)
          rotate_point(offset1,offset2,offset3,zero,angle_3,angle_4,axis);
        h_im = h_in + center*dilation_h + offset1;
        w_im = w_in + offset2;
        l_im = l_in + offset3;
        // if (index==10000 && (count < center || count ==center))
        // printf("count %d offset1 %.2f offset2 %.2f offset3 %.2f angle_1 %.2f angle_2 %.2f  \n",count,offset1,offset2,offset3,angle_1,angle_2);
      }
      else if ( axis ==1 )
      { 
        scalar_t offset1 = 0;
        scalar_t offset2 = (count-center)*dilation_w;
        scalar_t offset3 = 0;
        if (count == center)
          offset2 = 0;
        else if (count < center)
          rotate_point(offset1,offset2,offset3,angle_1,zero,angle_2,axis);
        else if (count > center)
          rotate_point(offset1,offset2,offset3,angle_3,zero,angle_4,axis);
        h_im = h_in + offset1;
        w_im = w_in + center*dilation_w + offset2;
        l_im = l_in + offset3;
      }
      else
      {
        scalar_t offset1 = 0;
        scalar_t offset2 = 0;
        scalar_t offset3 = (count-center)*dilation_l;
        if (count == center)
          offset3 = 0;
        else if (count < center)
          rotate_point(offset1,offset2,offset3,angle_1,angle_2,zero,axis);
        else if (count > center)
          rotate_point(offset1,offset2,offset3,angle_3,angle_4,zero,axis);
        h_im = h_in + offset1;
        w_im = w_in + offset2;
        l_im = l_in + center*dilation_l + offset3;
      }

      /* 边界检查 */
      if (h_im > height - 1) h_im = height - 1;
      if (w_im > width - 1) w_im = width - 1;
      if (l_im > length - 1) l_im = length - 1;
      if (h_im < 0) h_im = 0;
      if (w_im < 0) w_im = 0;
      if (l_im < 0) l_im = 0;
      ...
      {
        val = DAConv_im2col_trilinear(data_im_ptr, width, length, height, width, length, h_im, w_im,l_im);
      }
      *data_col_ptr = val;
...
void DAConv_im2col_cuda(
	...
	const int stride_h, const int stride_w, const int stride_l,
	const int dilation_h, const int dilation_w, const int dilation_l,
	const int deformable_group, at::Tensor data_col,const int axis)
{
	  // num_axes should be smaller than block size
  const int channel_per_deformable_group = channels / deformable_group;
  const int num_kernels = channels * batch_size * height_col * width_col *length_col;
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_im.scalar_type(), "DAConv_im2col_gpu_kernel", ([&] {
        const scalar_t *data_im_ = data_im.data_ptr<scalar_t>();
        scalar_t *data_offset_ = data_offset.data_ptr<scalar_t>();
        scalar_t *data_col_ = data_col.data_ptr<scalar_t>();

        DAConv_im2col_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
            ...
            height_col, width_col, length_col,data_col_,axis);
      }));

  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
    printf("error in DAConv_im2col: %s\n", cudaGetErrorString(err));
  }
}
...
int DAConv_forward_cuda(
        ...
        const int pad_h,const int pad_w,const int pad_l,
        const int dilation_h,const int dilation_w,const int dilation_l,
        const int group,const int deformable_group, const int in_step,const bool with_bias,const int axis){
// #define DEBUG
...
#ifdef DEBUG
  print_tensor_size("DAConv_forward_cuda---output size",output);
  print_tensor_size("DAConv_forward_cuda---input size",input);
  print_tensor_size("DAConv_forward_cuda---offset size",offset);
#endif
  ...
  offset =offset.view({batch / step, step,deformable_group*4,
  ...
  // print_tensor_sum("DAConv_forward_cuda---columns",columns.sum());
  ...
  offset=offset.view({batch,deformable_group * 4,height_out,width_out,length_out});
  return 0;
}


template <typename scalar_t>
__global__ void DAConv_gradient_gpu_kernel(
		const int n,const scalar_t *grad_col, const scalar_t *data_input,
		const scalar_t *angle_offset,scalar_t * columns,
        ...
        const int height_col, const int width_col, const int length_col,
        scalar_t * grad_input,scalar_t *grad_offset,const int axis){
// columns = {channels * kernel_h * kernel_w * kernel_l,step*height_out * width_out * length_out}
...
// grad_offset = {step,deformable_group * 3 * kernel_h * kernel_w * kernel_l,height_out,width_out,length_out}
  CUDA_KERNEL_LOOP(index, n)//channels*kernel_h * kernel_w * kernel_l* step * height_col * width_col * length_col;
  {
    ...
    int f = (index /step/ single_col_len )%(kernel_h * kernel_w * kernel_l);  // 计算index位于第哪一个卷积核
    int i=(f / kernel_l / kernel_w) % kernel_h;  // 计算卷积核的位置
    ...
    // if (index=10000)
    //   printf("index %d offset_group_index %d deformable_group %d k %d \n",index,offset_group_index,deformable_group,k);

    int angle_base_ptr=bpos*(deformable_group * 4 * single_col_len)
                  +offset_group_index*channel_per_deformable_group*single_col_len
                  +hpos_col*width_col*length_col+wpos_col*length_col+lpos_col;

    // 获取当前batch和channel组合的角度参数
    int angle_1_ptr = angle_base_ptr + 0*single_col_len;
    int angle_2_ptr = angle_base_ptr + 1*single_col_len;
    int angle_3_ptr = angle_base_ptr + 2*single_col_len;
    int angle_4_ptr = angle_base_ptr + 3*single_col_len;

    const scalar_t angle_1 = angle_offset[angle_1_ptr]; // 第一个角度参数
    const scalar_t angle_2 = angle_offset[angle_2_ptr]; // 第二个角度参数
    const scalar_t angle_3 = angle_offset[angle_3_ptr]; // 
    const scalar_t angle_4 = angle_offset[angle_4_ptr]; // 

    scalar_t real_offset_h = 0;
    scalar_t real_offset_w = 0;
    scalar_t real_offset_l = 0;
    scalar_t zero = 0;

    int hpos_in = hpos_col * stride_h - pad_h;
    int wpos_in = wpos_col * stride_w - pad_w;
    int lpos_in = lpos_col * stride_l - pad_l;
    
    int center = (kernel_h*kernel_w*kernel_l - 1)/2;
    if ( axis == 0 )
    {
      scalar_t offset1 = (i-center)*dilation_h;
      scalar_t offset2 = 0;
      scalar_t offset3 = 0;
      ...
      if (i == center)
        ...
        rotate_point(offset1,offset2,offset3,zero,angle_3,angle_4,axis);
      real_offset_h=hpos_in+center*dilation_h+offset1;
      real_offset_w=wpos_in+offset2;
      real_offset_l=lpos_in+offset3;
    }
    else if ( axis == 1 )
    {
      scalar_t offset1 = 0;
      scalar_t offset2 = (j-center)*dilation_w;
      scalar_t offset3 = 0;
      ...
      if (j == center)
        ...
        rotate_point(offset1,offset2,offset3,angle_3,zero,angle_4,axis);
      real_offset_h=hpos_in+offset1;
      real_offset_w=wpos_in+center*dilation_w+offset2;
      real_offset_l=lpos_in+offset3;
    }
    else 
    {
      scalar_t offset1 = 0;
      scalar_t offset2 = 0;
      scalar_t offset3 = (k-center)*dilation_l;
      ...
      real_offset_h=hpos_in+offset1;
      real_offset_w=wpos_in+offset2;
      real_offset_l=lpos_in+center*dilation_l+offset3;
    }
    ...
    /* 边界检查 */
    if (real_offset_h > height_input - 1) real_offset_h = height_input - 1;
    if (real_offset_w > width_input - 1) real_offset_w = width_input - 1;
    if (real_offset_l > length_input - 1) real_offset_l = length_input - 1;
    if (real_offset_h < 0) real_offset_h = 0;
    if (real_offset_w < 0) real_offset_w = 0;
    if (real_offset_l < 0) real_offset_l = 0;
    ...
    // 对于每个采样点，计算其相对于输入坐标的梯度
    scalar_t grad_v1 = w1*dval, grad_v2 = w2*dval, grad_v3 = w3*dval, grad_v4 = w4*dval;
    scalar_t grad_v5 = w5*dval, grad_v6 = w6*dval, grad_v7 = w7*dval, grad_v8 = w8*dval;// 对于每个采样点，计算其相对于输入坐标的梯度
    ...
    if (h_low_flag  && w_low_flag  && l_low_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_low * width_input*length_input + w_low* length_input+l_low,grad_v1);
    }
    ...
    if (h_low_flag && w_low_flag  && l_high_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_low * width_input*length_input + w_low* length_input+l_high,grad_v2);
    }
    ...
    if (h_low_flag && w_high_flag  && l_low_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_low * width_input*length_input + w_high* length_input+l_low,grad_v3);
    }
    ...
    if (h_low_flag && w_high_flag  && l_high_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_low * width_input*length_input + w_high* length_input+l_high,grad_v4);
    }
    ...
    if (h_high_flag  && w_low_flag  && l_low_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_high * width_input*length_input + w_low* length_input+l_low,grad_v5);
    }
    ...
    if (h_high_flag  && w_low_flag  && l_high_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_high * width_input*length_input + w_low* length_input+l_high,grad_v6);
    }
    ...
    if (h_high_flag  && w_high_flag  && l_low_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_high * width_input*length_input + w_high* length_input+l_low,grad_v7);
    }
    ...
    if (h_high_flag  && w_high_flag  && l_high_flag ){
      ...
      atomicAdd(grad_input+grad_input_base_ptr +h_high * width_input*length_input + w_high* length_input+l_high,grad_v8);
    }

    // 累加到grad_h, grad_w, grad_l
    scalar_t grad_h = (-1*(1-dw)*(1-dl)*v1-1*(1-dw)*dl*v2-1*dw*(1-dl)*v3-1*dw*dl*v4+(1-dw)*(1-dl)*v5+(1-dw)*dl*v6+dw*(1-dl)*v7+dw*dl*v8);
    scalar_t grad_w = (-1*(1-dh)*(1-dl)*v1-1*(1-dh)*dl*v2+(1-dh)*(1-dl)*v3+(1-dh)*dl*v4-1*dh*(1-dl)*v5-1*dh*dl*v6+dh*(1-dl)*v7+dh*dl*v8);
    scalar_t grad_l = (-1*(1-dh)*(1-dw)*v1+(1-dh)*(1-dw)*v2-1*(1-dh)*dw*v3+(1-dh)*dw*v4-1*dh*(1-dw)*v5+dh*(1-dw)*v6-1*dh*dw*v7+dh*dw*v8);
    ...
    if ( axis == 0 )
    {
      if (i < center)
      {
        scalar_t dxdtheta_y = -(i-center) * sinf(angle_1) * cosf(angle_2);
        scalar_t dxdtheta_z = -(i-center) * cosf(angle_1) * sinf(angle_2);

        scalar_t dydtheta_y = -(i-center) * sinf(angle_1) * sinf(angle_2);
        scalar_t dydtheta_z = (i-center) * cosf(angle_1) * cosf(angle_2);

        scalar_t dzdtheta_y = -(i-center) * cosf(angle_1);
        scalar_t dzdtheta_z = 0;

        scalar_t grad_theta_y = grad_h * dxdtheta_y + grad_w * dydtheta_y + grad_l * dzdtheta_y ;
        scalar_t grad_theta_z = grad_h * dxdtheta_z + grad_w * dydtheta_z + grad_l * dzdtheta_z ;
        atomicAdd(grad_offset + angle_1_ptr,
          grad_theta_y*dval);
        atomicAdd(grad_offset + angle_2_ptr,
          grad_theta_z*dval);
      }
      else if ( i > center )
      {
        scalar_t dxdtheta_y = -(i-center) * sinf(angle_3) * cosf(angle_4);
        scalar_t dxdtheta_z = -(i-center) * cosf(angle_3) * sinf(angle_4);

        scalar_t dydtheta_y = -(i-center) * sinf(angle_3) * sinf(angle_4);
        scalar_t dydtheta_z = (i-center) * cosf(angle_3) * cosf(angle_4);

        scalar_t dzdtheta_y = -(i-center) * cosf(angle_3);
        scalar_t dzdtheta_z = 0;

        scalar_t grad_theta_y = grad_h * dxdtheta_y + grad_w * dydtheta_y + grad_l * dzdtheta_y ;
        scalar_t grad_theta_z = grad_h * dxdtheta_z + grad_w * dydtheta_z + grad_l * dzdtheta_z ;

        atomicAdd(grad_offset + angle_3_ptr,
          grad_theta_y*dval);
        atomicAdd(grad_offset + angle_4_ptr,
          grad_theta_z*dval);
      }
    }
    else if ( axis == 1 )
    {
      if ( j < center)
      {
        scalar_t dxdtheta_x = (j-center) * sinf(angle_1) * sinf(angle_2);
        scalar_t dxdtheta_z = -(j-center) * cosf(angle_1) * cosf(angle_2);

        scalar_t dydtheta_x = -(j-center) * sinf(angle_1) * cosf(angle_2);
        scalar_t dydtheta_z = -(j-center) * cosf(angle_1) * sinf(angle_2);

        scalar_t dzdtheta_x = (j-center) * cosf(angle_1);
        scalar_t dzdtheta_z = 0;

        scalar_t grad_theta_x = grad_h * dxdtheta_x + grad_w * dydtheta_x + grad_l * dzdtheta_x;
        scalar_t grad_theta_z = grad_h * dxdtheta_z + grad_w * dydtheta_z + grad_l * dzdtheta_z;
      ...
      {
        scalar_t dxdtheta_x = (j-center) * sinf(angle_3) * sinf(angle_4);
        scalar_t dxdtheta_z = -(j-center) * cosf(angle_3) * cosf(angle_4);

        scalar_t dydtheta_x = -(j-center) * sinf(angle_3) * cosf(angle_4);
        scalar_t dydtheta_z = -(j-center) * cosf(angle_3) * sinf(angle_4);

        scalar_t dzdtheta_x = (j-center) * cosf(angle_3);
        scalar_t dzdtheta_z = 0;
    ...
    else
    {
      if ( k < center )
      {
        scalar_t dxdtheta_x = -(k-center) * sinf(angle_1) * sinf(angle_2);
        scalar_t dxdtheta_y = (k-center) * cosf(angle_1) * cosf(angle_2);

        scalar_t dydtheta_x = -(k-center) * cosf(angle_1);
        scalar_t dydtheta_y = 0;

        scalar_t dzdtheta_x = -(k-center) * sinf(angle_1) * cosf(angle_2);
        scalar_t dzdtheta_y = -(k-center) * cosf(angle_1) * sinf(angle_2);
      ...
      {
        scalar_t dxdtheta_x = -(k-center) * sinf(angle_3) * sinf(angle_4);
        scalar_t dxdtheta_y = (k-center) * cosf(angle_3) * cosf(angle_4);

        scalar_t dydtheta_x = -(k-center) * cosf(angle_3);
        scalar_t dydtheta_y = 0;

        scalar_t dzdtheta_x = -(k-center) * sinf(angle_3) * cosf(angle_4);
        scalar_t dzdtheta_y = -(k-center) * cosf(angle_3) * sinf(angle_4);
...
void DAConv_gradient_cuda(
    ...
    at::Tensor grad_input, at::Tensor grad_offset,const int axis)
{
  const int num_kernels =channels*height_col * width_col * length_col * kernel_h * kernel_w * kernel_l *step;
  const int channel_per_deformable_group = 4;

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        grad_col.scalar_type(), "DAConv_gradient_gpu_kernel", ([&] {
        ...
        DAConv_gradient_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
            ...
            grad_input_,grad_offset_,axis);
      }));
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
    printf("error in DAConv_gradient_cuda: %s\n", cudaGetErrorString(err));
  }
}
...
int DAConv_backward_cuda(
    ...
    const int pad_h,const int pad_w,const int pad_l,
    const int dilation_h,const int dilation_w,const int dilation_l,
    const int group, int deformable_group,const int in_step,const bool with_bias,const int axis) {
  TORCH_CHECK(input.is_contiguous(), "input tensor has to be contiguous");
  TORCH_CHECK(weight.is_contiguous(), "weight tensor has to be contiguous");
  TORCH_CHECK(bias.is_contiguous(), "bias tensor has to be contiguous");
  TORCH_CHECK(offset.is_contiguous(), "offset tensor has to be contiguous");
  ...
  offset=offset.view({batch/step,step,
	  deformable_group * 4,height_out,width_out,length_out});
  ...
  for (int b = 0; b < batch/step; b++) {
    ...
    for (int g = 0; g < group; g++) {
    	grad_columns[g].addmm_(weight[g].flatten(1).transpose(0, 1),
                        grad_output[b][g].flatten(1), 1.0f, 1.0f);
    }
    ...
    DAConv_gradient_cuda(
    		...
    		grad_input[b],grad_offset[b],axis);
    columns = columns.view({group, columns.size(0) / group, columns.size(1)});
    // print_tensor_sum("DAConv_backward_cuda---columns",columns.sum());
  ...
  offset=offset.view({batch,deformable_group * 4,
	  	  	  	  	  height_out,width_out,length_out});
  grad_offset=grad_offset.view({batch,deformable_group * 4,
  	  	  	  	  	  height_out,width_out,length_out});
  return 0;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("DAConv_forward_cuda", &DAConv_forward_cuda,
        "DAConv_forward_cuda");
  m.def("DAConv_backward_cuda", &DAConv_backward_cuda,
        "DAConv_backward_cuda");
}
