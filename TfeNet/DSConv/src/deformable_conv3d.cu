...
__device__ scalar_t DSConv_im2col_trilinear(
...
template <typename scalar_t>
__global__ void DSConv_im2col_gpu_kernel(
		const int n, const scalar_t *data_im,scalar_t *data_offset,scalar_t *snake,
...
//  columns = {channels * kernel_h * kernel_w * kernel_l,step * height_out * width_out * length_out}
    ...
    scalar_t *data_offset_ptr = data_offset
    		+(b_col * deformable_group + deformable_group_index)
    		* 2 * kernel_h * kernel_w * kernel_l * height_col * width_col * length_col;// 1 18 100 100

    scalar_t *snake_ptr = snake
    		+(b_col * deformable_group + deformable_group_index)
    		* 2 * kernel_h * kernel_w * kernel_l * height_col * width_col * length_col;// 1 18 100 100

    /*********************************  蛇形偏移变换 *********************************/ 

    scalar_t offset_sum_1 = 0;
    scalar_t offset_sum_2 = 0;
    int snake_center = (kernel_h * kernel_w * kernel_l - 1)/2;
    ...
    for (int k=0;k<kernel_l;++k)
    {
      ...
      if ( axis == 0 ) // x轴
      {
        if (i == snake_center)
        {
          int f=snake_center*kernel_w*kernel_l + j*kernel_l+k;
          int data_offset_1_ptr = (2*f) * height_col * width_col * length_col+ h_col* width_col * length_col+ w_col* length_col + l_col;
          int data_offset_2_ptr = (2*f+1) * height_col * width_col* length_col + h_col* width_col* length_col + w_col* length_col + l_col;
          
          const scalar_t offset_1 = data_offset_ptr[data_offset_1_ptr];
          const scalar_t offset_2 = data_offset_ptr[data_offset_2_ptr];

          offset_sum_1 = 0;
          offset_sum_2 = 0;

          snake_ptr[data_offset_1_ptr] = offset_sum_1;
          snake_ptr[data_offset_2_ptr] = offset_sum_2;

          h_im = h_in + i * dilation_h;
          w_im = w_in + j * dilation_w + offset_sum_1;
          l_im = l_in + k * dilation_l + offset_sum_2;
          ...
          {
            val = DSConv_im2col_trilinear(data_im_ptr, width, length, height, width, length, h_im, w_im,l_im);
          }
          scalar_t *data_col_ptr_current = data_col_ptr + i*(batch_size * height_col * width_col* length_col);
          *data_col_ptr_current = val;
        }
        else if (i < snake_center)
        {
          offset_sum_1 = 0;
          offset_sum_2 = 0;
          for (int n=i;n<snake_center;n++)
          {
            int f=n*kernel_w*kernel_l + j*kernel_l+k;
            int data_offset_1_ptr = (2*f) * height_col * width_col * length_col+ h_col* width_col * length_col+ w_col* length_col + l_col;
            int data_offset_2_ptr = (2*f+1) * height_col * width_col* length_col + h_col* width_col* length_col + w_col* length_col + l_col;
            ...
            offset_sum_1 += offset_1;
            offset_sum_2 += offset_2;
          }
      ...
      else if ( axis ==1 )
      {
        if (j == snake_center)
        {
          int f=i*kernel_w*kernel_l + snake_center*kernel_l+k;
          int data_offset_1_ptr = (2*f) * height_col * width_col * length_col+ h_col* width_col * length_col+ w_col* length_col + l_col;
          int data_offset_2_ptr = (2*f+1) * height_col * width_col* length_col + h_col* width_col* length_col + w_col* length_col + l_col;
          ...
          h_im = h_in + i * dilation_h + offset_sum_1;
          w_im = w_in + j * dilation_w;
          l_im = l_in + k * dilation_l + offset_sum_2; 
          ...
          scalar_t *data_col_ptr_current = data_col_ptr + j*(batch_size * height_col * width_col* length_col);
          *data_col_ptr_current = val;
        }
        else if (j < snake_center)
        {
          offset_sum_1 = 0;
          offset_sum_2 = 0;
          for (int n=j;n<snake_center;n++)
          {
            int f=i*kernel_w*kernel_l + n*kernel_l+k;
            int data_offset_1_ptr = (2*f) * height_col * width_col * length_col+ h_col* width_col * length_col+ w_col* length_col + l_col;
            int data_offset_2_ptr = (2*f+1) * height_col * width_col* length_col + h_col* width_col* length_col + w_col* length_col + l_col;
      ...
      else
      {
        if (k == snake_center)
        {
          ...
          h_im = h_in + i * dilation_h + offset_sum_1;
          w_im = w_in + j * dilation_w + offset_sum_2;
          l_im = l_in + k * dilation_l;
          ...
          scalar_t *data_col_ptr_current = data_col_ptr + k*(batch_size * height_col * width_col* length_col);
          *data_col_ptr_current = val;
        }
        else if (k < snake_center)
        {
          offset_sum_1 = 0;
          offset_sum_2 = 0;
          for (int n=k;n<snake_center;n++)
          {
            int f=i*kernel_w*kernel_l + j*kernel_l+n;
            int data_offset_1_ptr = (2*f) * height_col * width_col * length_col+ h_col* width_col * length_col+ w_col* length_col + l_col;
            int data_offset_2_ptr = (2*f+1) * height_col * width_col* length_col + h_col* width_col* length_col + w_col* length_col + l_col;
...
void DSConv_im2col_cuda(
    at::Tensor data_im, at::Tensor data_offset, at::Tensor snake,
	...
	const int deformable_group, at::Tensor data_col,const int axis)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_im.scalar_type(), "DSConv_im2col_gpu_kernel", ([&] {
        const scalar_t *data_im_ = data_im.data_ptr<scalar_t>();
        scalar_t *data_offset_ = data_offset.data_ptr<scalar_t>();
        scalar_t *snake_ = snake.data_ptr<scalar_t>();
        scalar_t *data_col_ = data_col.data_ptr<scalar_t>();

        DSConv_im2col_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
            num_kernels, data_im_, data_offset_,snake_,
  ...
  if (err != cudaSuccess)
  {
    printf("error in DSConv_im2col: %s\n", cudaGetErrorString(err));
  }
}

int DSConv_forward_cuda(
		at::Tensor input, at::Tensor weight,at::Tensor bias,
        at::Tensor offset,at::Tensor snake,at::Tensor output,
        const int kernel_h,const int kernel_w,const int kernel_l,
        const int stride_h,const int stride_w,const int stride_l,
        const int pad_h,const int pad_w,const int pad_l,
        const int dilation_h,const int dilation_w,const int dilation_l,
...
//#define DEBUG
  ...
  TORCH_CHECK(snake.is_contiguous(), "snake tensor has to be contiguous");
...
#ifdef DEBUG
  print_tensor_size("DSConv_forward_cuda---output size",output);
  print_tensor_size("DSConv_forward_cuda---input size",input);
  print_tensor_size("DSConv_forward_cuda---offset size",offset);
#endif
  ...
  offset =offset.view({batch / step, step,deformable_group*2* kernel_h * kernel_w * kernel_l,
	  	  	  	  	  height_out, width_out,length_out});
  snake = snake.view({batch / step, step,deformable_group*2* kernel_h * kernel_w * kernel_l,
	  	  	  	  	  height_out, width_out,length_out});
  //divide into group
  ...
  for (int b = 0; b < batch / step; b++) {
      columns.fill_(0);
	  DSConv_im2col_cuda(
			  input[b], offset[b],snake[b],step,channels,
			  height,width,length,
			  height_out, width_out, length_out,
  ...
  offset=offset.view({batch,deformable_group * 2 *kernel_h*kernel_w*kernel_l,height_out,width_out,length_out});
  snake=snake.view({batch,deformable_group * 2 *kernel_h*kernel_w*kernel_l,height_out,width_out,length_out});
  return 0;
}


template <typename scalar_t>
__global__ void DSConv_gradient_gpu_kernel(
		const int n,const scalar_t *grad_col, const scalar_t *data_input,
		const scalar_t *data_offset,const scalar_t *snake,scalar_t * columns,
...
// grad_offset = {step,deformable_group * 3 * kernel_h * kernel_w * kernel_l,height_out,width_out,length_out}
  CUDA_KERNEL_LOOP(index, n)//channels*kernel_h * kernel_w * kernel_l* step * height_col * width_col * length_col;
  {
    ...
    int offset_base_ptr=bpos*(deformable_group * 2 * kernel_h * kernel_w * kernel_l*single_col_len)
    ...
    if ( axis == 0 )
    {
      ...
      for (int current=init_num;current<loop_num;current++)
      {
        if ( i < snake_center)
        {
          int f_=(current)*kernel_w*kernel_l + j*kernel_l+k;
          offset_w_ptr=offset_base_ptr+2*f_*single_col_len;
          offset_l_ptr=offset_base_ptr+(2*f_+1)*single_col_len; 
          hpos_in = hpos_col * stride_h - pad_h + (current) * dilation_h;
          wpos_in = wpos_col * stride_w - pad_w + (j) * dilation_w;
          lpos_in = lpos_col * stride_l - pad_l + (k) * dilation_l;
          dval=grad_col[index - i + count];
          count++;
        }
        else if ( i > snake_center)
        {
          ...
          wpos_in = wpos_col * stride_w - pad_w + (j) * dilation_w;
          lpos_in = lpos_col * stride_l - pad_l + (k) * dilation_l;
          dval=grad_col[index + count];
          count++;
        }

        const scalar_t snake_w = snake[offset_w_ptr];
        const scalar_t snake_l = snake[offset_l_ptr];

        real_offset_h=0+hpos_in;
        real_offset_w=snake_w+wpos_in;
        real_offset_l=snake_l+lpos_in;
        ...
        // scalar_t dvdh = (-1*(1-dw)*(1-dl)*v1-1*(1-dw)*dl*v2-1*dw*(1-dl)*v3-1*dw*dl*v4+(1-dw)*(1-dl)*v5+(1-dw)*dl*v6+dw*(1-dl)*v7+dw*dl*v8);
        scalar_t dvdw = (-1*(1-dh)*(1-dl)*v1-1*(1-dh)*dl*v2+(1-dh)*(1-dl)*v3+(1-dh)*dl*v4-1*dh*(1-dl)*v5-1*dh*dl*v6+dh*(1-dl)*v7+dh*dl*v8)*dval;
        scalar_t dvdl = (-1*(1-dh)*(1-dw)*v1+(1-dh)*(1-dw)*v2-1*(1-dh)*dw*v3+(1-dh)*dw*v4-1*dh*(1-dw)*v5+dh*(1-dw)*v6-1*dh*dw*v7+dh*dw*v8)*dval;
        dvdw_sum += dvdw;
        dvdl_sum += dvdl;
      }
      offset_w_ptr=offset_base_ptr+2*f*single_col_len;
      offset_l_ptr=offset_base_ptr+(2*f+1)*single_col_len; 
      ...
      scalar_t snake_w=snake[offset_w_ptr];
      scalar_t snake_l=snake[offset_l_ptr];
    ...
    else if ( axis == 1 )
    {
      ...
      for (int current=init_num;current<loop_num;current++)
      {
        if ( j < snake_center)
        {
          int f_=i*kernel_w*kernel_l + (current)*kernel_l+k;
          offset_h_ptr=offset_base_ptr+2*f_*single_col_len;
          offset_l_ptr=offset_base_ptr+(2*f_+1)*single_col_len;
          hpos_in = hpos_col * stride_h - pad_h + (i) * dilation_h;
          wpos_in = wpos_col * stride_w - pad_w + (current) * dilation_w;
          lpos_in = lpos_col * stride_l - pad_l + (k) * dilation_l; 
          dval=grad_col[index - j + count];
          count++;
        }
        else if ( j > snake_center)
        {
        ...
        const scalar_t snake_h = snake[offset_h_ptr];
        const scalar_t snake_l = snake[offset_l_ptr];   

        real_offset_h=snake_h+hpos_in;
        real_offset_w=0+wpos_in;
        real_offset_l=snake_l+lpos_in;
        ...
        scalar_t dvdh = (-1*(1-dw)*(1-dl)*v1-1*(1-dw)*dl*v2-1*dw*(1-dl)*v3-1*dw*dl*v4+(1-dw)*(1-dl)*v5+(1-dw)*dl*v6+dw*(1-dl)*v7+dw*dl*v8)*dval;
        // scalar_t dvdw = (-1*(1-dh)*(1-dl)*v1-1*(1-dh)*dl*v2+(1-dh)*(1-dl)*v3+(1-dh)*dl*v4-1*dh*(1-dl)*v5-1*dh*dl*v6+dh*(1-dl)*v7+dh*dl*v8);
        scalar_t dvdl = (-1*(1-dh)*(1-dw)*v1+(1-dh)*(1-dw)*v2-1*(1-dh)*dw*v3+(1-dh)*dw*v4-1*dh*(1-dw)*v5+dh*(1-dw)*v6-1*dh*dw*v7+dh*dw*v8)*dval;
        dvdh_sum += dvdh;
        dvdl_sum += dvdl;
      }
      offset_h_ptr=offset_base_ptr+2*f*single_col_len;
      offset_l_ptr=offset_base_ptr+(2*f+1)*single_col_len;   
      ...
      scalar_t snake_h=snake[offset_h_ptr];
      scalar_t snake_l=snake[offset_l_ptr];
    ...
    else
    {
      ...
      for (int current=init_num;current<loop_num;current++)
      {
        if ( k < snake_center)
        {
          int f_=i*kernel_w*kernel_l + j*kernel_l+(current);
          offset_h_ptr=offset_base_ptr+2*f_*single_col_len;
          offset_w_ptr=offset_base_ptr+(2*f_+1)*single_col_len; 
          hpos_in = hpos_col * stride_h - pad_h + (i) * dilation_h;
          wpos_in = wpos_col * stride_w - pad_w + (j) * dilation_w;
          lpos_in = lpos_col * stride_l - pad_l + (current) * dilation_l;
          dval=grad_col[index - k + count];
          count++;
        }
        else if ( k > snake_center)
        {
        ...
        scalar_t dvdh = (-1*(1-dw)*(1-dl)*v1-1*(1-dw)*dl*v2-1*dw*(1-dl)*v3-1*dw*dl*v4+(1-dw)*(1-dl)*v5+(1-dw)*dl*v6+dw*(1-dl)*v7+dw*dl*v8)*dval;
        scalar_t dvdw = (-1*(1-dh)*(1-dl)*v1-1*(1-dh)*dl*v2+(1-dh)*(1-dl)*v3+(1-dh)*dl*v4-1*dh*(1-dl)*v5-1*dh*dl*v6+dh*(1-dl)*v7+dh*dl*v8)*dval;
        // scalar_t dvdl = (-1*(1-dh)*(1-dw)*v1+(1-dh)*(1-dw)*v2-1*(1-dh)*dw*v3+(1-dh)*dw*v4-1*dh*(1-dw)*v5+dh*(1-dw)*v6-1*dh*dw*v7+dh*dw*v8);
        dvdh_sum += dvdh;
        dvdw_sum += dvdw;
      }
      offset_h_ptr=offset_base_ptr+2*f*single_col_len;
      offset_w_ptr=offset_base_ptr+(2*f+1)*single_col_len;   
...
void DSConv_gradient_cuda(
    at::Tensor grad_col,at::Tensor data_input, at::Tensor data_offset,at::Tensor snake,at::Tensor columns,
    ...
    at::Tensor grad_input, at::Tensor grad_offset,const int axis)
{
  ...
  const int channel_per_deformable_group =2 * kernel_h * kernel_w * kernel_l;

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        grad_col.scalar_type(), "DSConv_gradient_gpu_kernel", ([&] {
        const scalar_t *grad_col_ = grad_col.data<scalar_t>();
        const scalar_t *data_input_ = data_input.data<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data<scalar_t>();
        const scalar_t *snake_ = snake.data<scalar_t>();
        scalar_t *columns_ = columns.data<scalar_t>();
        scalar_t *grad_input_ = grad_input.data<scalar_t>();
        scalar_t *grad_offset_ = grad_offset.data<scalar_t>();

        DSConv_gradient_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
            num_kernels, grad_col_, data_input_, data_offset_,snake_,columns_,
  ...
  if (err != cudaSuccess)
  {
    printf("error in DSConv_gradient_cuda: %s\n", cudaGetErrorString(err));
  }
}

int DSConv_backward_cuda(
    at::Tensor input, at::Tensor weight, at::Tensor bias,at::Tensor offset,at::Tensor snake,
  ...
  offset=offset.view({batch/step,step,
	  deformable_group * 2 * kernel_h * kernel_w * kernel_l,height_out,width_out,length_out});
  snake=snake.view({batch/step,step,
	  deformable_group * 2 * kernel_h * kernel_w * kernel_l,height_out,width_out,length_out});
  ...
  for (int b = 0; b < batch/step; b++) {
    ...
    columns.fill_(0);
    DSConv_gradient_cuda(
    		grad_columns, input[b], offset[b],snake[b], columns,
  ...
  offset=offset.view({batch,deformable_group * 2 * kernel_h * kernel_w *kernel_l,
	  	  	  	  	  height_out,width_out,length_out});
  snake=snake.view({batch,deformable_group * 2 * kernel_h * kernel_w *kernel_l,
	  	  	  	  	  height_out,width_out,length_out});
  grad_offset=grad_offset.view({batch,deformable_group * 2 * kernel_h * kernel_w *kernel_l,
  	  	  	  	  	  height_out,width_out,length_out});
  return 0;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("DSConv_forward_cuda", &DSConv_forward_cuda,
        "DSConv_forward_cuda");
  m.def("DSConv_backward_cuda", &DSConv_backward_cuda,
        "DSConv_backward_cuda");
}
